...
func (s *checkLogsTestSuite) TestLogContains() {
	...
	logsCheck, err := s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, "did not find expected content 'id \"920300\"'")

	s.check.SetLogContains(`SOMETHING`)
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.False(logsCheck, "found something that is not there")

	s.check.SetLogContains("")
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, "empty LogContains should return true")
}
...
func (s *checkLogsTestSuite) TestNoLogContains() {
	...
	logsCheck, err := s.check.AssertLogs()
	s.Require().NoError(err)
	s.False(logsCheck, "did not find expected content")

	s.check.SetNoLogContains("SOMETHING")
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, "found something that is not there")

	s.check.SetNoLogContains("")
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, "should return true when empty string is passed")
}
...
func (s *checkLogsTestSuite) TestAssertLogMatchRegex() {
	...
	logsCheck, err := s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, `did not find expected content 'id\s"920300"'`)
...
func (s *checkLogsTestSuite) TestAssertLogNoMatchRegex() {
	...
	logsCheck, err := s.check.AssertLogs()
	s.Require().NoError(err)
	s.False(logsCheck, `expected to find 'id\s"920300"'`)

	s.check.expected.Log.NoMatchRegex = `SOMETHING`
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, "expected to _not_ find SOMETHING")
...
func (s *checkLogsTestSuite) TestAssertLogExpectIds_Multiple() {
	...
	logsCheck, err := s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, "Expected to find '920300'")

	s.check.expected.Log.ExpectIds = []uint{920300, 949110}
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, "Expected to find all IDs")
}
...
func (s *checkLogsTestSuite) TestAssertLogExpectIds_Subset() {
	...
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.False(logsCheck, "Did not expect to find '123'")
}
...
func (s *checkLogsTestSuite) TestAssertLogNoExpectIds_Multiple() {
	...
	logsCheck, err := s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, "Did not expect to find '123'")

	s.check.expected.Log.NoExpectIds = []uint{123, 456}
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.True(logsCheck, "Did not expect to find any of the IDs")
}
...
func (s *checkLogsTestSuite) TestAssertLogNoExpectIds_Subset() {
	...
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.False(logsCheck, "Expected to find '920300'")
}
...
func (s *checkLogsTestSuite) TestAssertLogIsolated() {
	...
	logsCheck, err = s.check.AssertLogs()
	s.Require().NoError(err)
	s.False(logsCheck, "Expected to find multiple IDs")
}
